...
test('HumanStandardToken balances are tracked', function (t) {
  ...
  .then(() => {
    ...
    const data = tracked
    t.equal(data.string, '8.900', 'represents decimals')
...
test('HumanStandardToken balance changes are emitted', function (t) {
  ...
  tokenTracker.on('update', (data) => {
    ...
    updateCounter++
    if (updateCounter < 2) {
      return t.equal(tracked.string, '8.900', 'initial balance loaded from last test')
    }

    t.equal(tracked.symbol, 'DBX', 'symbol retrieved')
    t.equal(tracked.string, '7.900', 'balance updated')
    tokenTracker.stop()
    t.end()
  })
...
test('HumanStandardToken non balance changes are not emitted', function (t) {
  ...
  tokenTracker.on('update', (data) => {
    ...
    updateCounter++
    if (updateCounter < 2) {
      return t.equal(tracked.string, '7.900', 'initial balance loaded from last test')
    }
